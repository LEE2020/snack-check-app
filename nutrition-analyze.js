/**
 * 仅根据图中识别出的文字（配料表、营养成分表）分析，不依赖任何食物名称数据库。
 * 结合糖尿病患者摄入标准，给出含糖量等级与食用量提醒。
  ...
  function analyzeForDiabetic(ocrText) {
    ...
    if (!ocrText || ocrText.trim().length < 2) {
      return {
        ...
        portionAdvice: "",
        summary: "未识别到有效文字",
        details: "本结果仅依据图中识别出的配料与营养成分分析，不使用食物数据库。请拍摄包含「营养成分表」或「配料表」的包装背面/侧面，确保文字清晰、光线充足。",
        snippet: ""
      };
    }
    ...
    if (scores.length === 0) {
      return {
        badge: "caution",
        sugarLevel: "未从图中解析到糖或碳水数据",
        portionAdvice: "请对准营养成分表重拍，或参考下方「识别到的文字」自行判断。",
        summary: "仅根据图中文字分析：未解析出糖/碳水化合物数值",
        details: "本结果仅依据识别出的文字分析，不使用数据库匹配。图中可能无完整「营养成分表」或「配料表」，或文字不清晰。请对准包装上的成分表重拍，或查看下方识别文字。",
        snippet: snippet
      };
    }
